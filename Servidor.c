...
int main(int argc, char *argv[])
{
	...
	for(;;){
		...
		while(terminar==0){
			...
			//buffer peticion, obteenmos el tamaño del mensaje
			printf ("Recibido\n");
			...
			if(codigo==3){
				...
				if (altura > 1.70)
					sprintf(respuesta,"%s de pequeño tomabas actimels",nombre);
				if (altura < 1.70)
					sprintf(respuesta,"%s no te preocupes ya pegaras el estiron",nombre);
